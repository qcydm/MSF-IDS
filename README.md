...
## Overview
This project introduces an innovative Unsupervised Intrusion Detection System (UNIDS) that employs a multi-scale spatial-frequency lens approach for the identification of network anomalies. Built using the PyTorch framework, this system is designed to operate without the need for labeled data, making it highly adaptable to various network environments. A key feature of this system is its ability to analyze network traffic through a combination of spatial and frequency encoders, which provide a deeper understanding of the network's structure and dynamics.We also provide a NAPH application for visualization. You can download the compressed package and follow some steps to run it.
