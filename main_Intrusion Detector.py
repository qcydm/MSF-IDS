...
def pretrain(epoch, save_epoch):
    ...
    if epoch == save_epoch:
        ...
        torch.save(embeddings_save, "edge_embeddings_train" + args.token + ".pt")
        labels = torch.cat(labels_train, dim=0)
        torch.save(labels, "edge_labels_train" + args.token + ".pt")
...
def SE_layer(zs, ys):
    ...
    for iters in range(args.iterations):
        ...
        if scores[0] > args.SE_THRESHOLD:
            break
    print(len(from_list))
    np.save("from_list" + args.token + ".npy", from_list)
    np.save("to_list" + args.token + ".npy", to_list)
    np.save("val_list" + args.token + ".npy", val_list)
...
def main():
    ...
    zs = torch.load("edge_embeddings_train" + args.token + ".pt")
    ys = torch.load("edge_labels_train" + args.token + ".pt")
    print(zs.shape)
    print(ys.shape)
    ys_np = ys.cpu().numpy()
    with open(LOG_PATH, "a") as f:
        f.write(log_str)
    SE_layer(zs, ys)
    from_list = np.load("from_list" + args.token + ".npy")
    val_list = np.load("val_list" + args.token + ".npy")
    to_list = np.load("to_list" + args.token + ".npy")
    with open(LOG_PATH, "a") as f:
        f.write("start train()")
    conf_list = []
    torch.cuda.empty_cache()
